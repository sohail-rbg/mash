...
export default function StatusBar({ foods, error, spinning, loading, selectedMode, checkedIngredients, remainingCount }) {
  const cfg = (() => {
    ...
    if (!selectedMode) return { // Display when no mode is selected
      bg: "bg-gray-100/10 dark:bg-white/[0.04]", border: "border-gray-300/40 dark:border-white/[0.08]",
      dot: "bg-gray-400 dark:bg-white/10",
      text: "text-gray-500 dark:text-white/35",
      label: "Select a mode to begin",
    };

    return { // Ready to spin state
