...
export default async function SharePage({ searchParams }) {
  ...
  const foodName = resolvedParams?.name || 'Delicious Food';
  const foodDescription = resolvedParams?.description || 'A tasty food card shared from Mash';
  const foodImage =
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#fb923c_0%,#111827_58%)] px-4 py-12 text-white">
      <div className="mx-auto max-w-md rounded-[32px] border border-white/10 bg-white/5 p-4 shadow-2xl backdrop-blur-xl">
        <div className="overflow-hidden rounded-[28px] bg-black/10">
          <div className="relative aspect-[4/5] w-full overflow-hidden">
            ...
            <div className="absolute inset-x-0 bottom-0 p-5">
              ...
              <p className="mt-2 line-clamp-3 text-sm text-white/80">{foodDescription}</p>
            </div>
          </div>
        </div>
      </div>
